...
export interface IStorage {
  ...
  // Body Measurements - tape measure tracking
  createBodyMeasurement(entry: InsertBodyMeasurement): Promise<BodyMeasurement>;
  updateBodyMeasurement(id: string, entry: UpdateBodyMeasurement): Promise<BodyMeasurement | undefined>;
  ...
  // Supplements - supplement tracking with URL preview
  createSupplement(entry: InsertSupplement): Promise<Supplement>;
...
export class MemStorage implements IStorage {
  ...
  // Daily workout status methods (stubs for MemStorage)
  async getDailyWorkoutStatus(category: string, date: Date): Promise<DailyWorkoutStatus | undefined> {
    return undefined; // Not implemented for in-memory storage
  }

  async setDailyWorkoutStatus(category: string, date: Date, isCompleted: boolean): Promise<DailyWorkoutStatus> {
    const id = randomUUID();
    const now = new Date();
    const status: DailyWorkoutStatus = {
      id,
      category,
      date,
...
export class DatabaseStorage implements IStorage {
  ...
  // Daily workout status tracking methods
  async getDailyWorkoutStatus(category: string, date: Date): Promise<DailyWorkoutStatus | undefined> {
    const startOfDay = new Date(date);
    startOfDay.setHours(0, 0, 0, 0);
    const endOfDay = new Date(date);
    ...
    const [result] = await db.select()
      .from(dailyWorkoutStatus)
      .where(and(
        eq(dailyWorkoutStatus.category, category),
        gte(dailyWorkoutStatus.date, startOfDay),
        lte(dailyWorkoutStatus.date, endOfDay)
      ));
  ...
  async setDailyWorkoutStatus(category: string, date: Date, isCompleted: boolean): Promise<DailyWorkoutStatus> {
    const existing = await this.getDailyWorkoutStatus(category, date);
    
    if (existing) {
      const [updated] = await db.update(dailyWorkoutStatus)
        .set({
          isCompleted: isCompleted ? 1 : 0,
          updatedAt: new Date()
        })
        .where(eq(dailyWorkoutStatus.id, existing.id))
        .returning();
      return updated;
    } else {
      const [created] = await db.insert(dailyWorkoutStatus)
        .values({
          category,
          date,
          isCompleted: isCompleted ? 1 : 0
  ...
  async createBodyMeasurement(entry: InsertBodyMeasurement): Promise<BodyMeasurement> {
    const [created] = await db.insert(bodyMeasurements)
      .values({
        ...entry,
        createdAt: new Date(),
      })
  ...
  async updateBodyMeasurement(id: string, entry: UpdateBodyMeasurement): Promise<BodyMeasurement | undefined> {
  ...
  async getLatestBodyMeasurement(): Promise<BodyMeasurement | undefined> {
  ...
  // Supplements - supplement tracking with URL preview
  async createSupplement(entry: InsertSupplement): Promise<Supplement> {
    const [created] = await db.insert(supplements)
      .values({
        ...entry,
        createdAt: new Date(),
        updatedAt: new Date(),
  ...
  async updateSupplement(id: string, entry: UpdateSupplement): Promise<Supplement | undefined> {
  ...
  async deleteSupplement(id: string): Promise<boolean> {
    const result = await db.delete(supplements)
      .where(eq(supplements.id, id));
    return result.rowCount > 0;
  }
