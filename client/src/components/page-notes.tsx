...
export default function PageNotes({ 
  ...
  placeholder = "Add your notes and instructions here..."
}: PageNotesProps) {
  const [isCollapsed, setIsCollapsed] = useState(true);
  const [notes, setNotes] = useState("");
  const { toast } = useToast();

  // Query to get existing notes for this page
  const { data: notesData, isLoading } = useQuery<{ notes: string }>({
    queryKey: ["/api/page-notes", page],
    queryFn: async () => {
      const response = await apiRequest("GET", `/api/page-notes/${page}`);
  ...
  const saveNotesMutation = useMutation({
    mutationFn: async (notesToSave: string) => {
      const response = await apiRequest("POST", `/api/page-notes/${page}`, { notes: notesToSave });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/page-notes", page] });
    },
    onError: () => {
      toast({
        title: "Error",
        ...
        <CollapsibleContent>
          <CardContent className="pt-0">
            <Textarea
              ...
              disabled={isLoading}
              data-testid={`textarea-notes-${page}`}
            />
          </CardContent>
        </CollapsibleContent>
      </Collapsible>
